...
(Agregar como sección la identificación)

Con el objetivo de seleccionar la especificación más adecuada se utilizará la función `ARIMA()` del paquete `fable`, que permite realizar una selección automática del mejor modelo basandose en el algoritmo de Hyndman-Khandakar, el cuál combina test de raíz unitaria, minimización del criterio de información $AICc$ y la estimación mediante el método de máxima verosimilitud para obtener el modelo ARIMA adecuado. (Agrego como funciona el algoritmo?)
...
Esta misma contiene una serie de características que la convierten en un caso adecuado para evaluar y comparar metodologías de pronóstico diseñadas para series interrumpidas. En particular, permiten analizar la capacidad de los distintos enfoques para adaptarse a cambios estructurales abruptos y generar pronósticos confiables en contextos de alta incertidumbre.
...
La serie en estudio corresponde al número de pasajeros transportados de forma mensual por el sistema de Transporte Urbano de Pasajeros (TUP) en la ciudad de Rosario. (Fuente de los datos).
...
En la @fig-serie1 se puede observar que la serie en estudio registra un conjunto de observaciones atípicas durante los años 2020 y 2021, consecuencia directa de las restricciones sanitarias impuestas por la pandemia de COVID-19 (evento disruptivo), así como una recuperación no lineal e incompleta en comparación con los niveles pre-pandemia.

Por otro lado, la @fig-serie2 presenta una estacionalidad marcada particularmente en los meses asociados a las vacaciones de verano (diciembre, enero y febrero) y de invierno (julio), y una tendencia negativa a través de los años, esto puede ser una consecuencia directa de la pandemia de COVID-19.
...
Con respecto al análisis de dispersión, se puede identificar en la @fig-serie3 una leve tendecia decreciente en la media y que la variabilidad no parece ser constante año a año. Estos patrones dan un indicio de que la serie no es estacionaria. Para solucionar la falta de estacionariedad en variancia, se transforma la variable utilizando la transformación de Box-Cox, el valor de $\lambda = 0$ fue obtenido a partir de minimizar el coeficiente de variación en la @tbl-bc, por lo que la transformación que se utilizó fue $y^{(\lambda)} = ln(y)$.
...
Cabe destacar que, la @fig-serie4 muestra valores positivos elevados en los primeros rezagos, con un decrecimiento gradual a medida que aumenta el rezago, esto sugiere que la serie podría no ser estrictamente estacionaria. En estos casos, es habitual considerar la aplicación de diferenciación para estabilizar la media de la serie. 

Asimismo, la @fig-serie5 presenta un pico significativo en el primer rezago y algunos rezagos adicionales con valores estadísticamente significativos, mientras que la mayoría de los rezagos posteriores se mantienen dentro de las bandas de confianza.
