...
En este contexto, la presente tesina propone evaluar el desempeño de distintas metodologías de pronóstico para series temporales interrumpidas mediante su aplicación a la serie mensual de pasajeros del Transporte Urbano de Pasajeros (TUP) de la ciudad de Rosario. A través de la comparación de estas estrategias en el periodo disruptivo y la etapa posterior, se busca generar evidencia empírica que contribuya a la comprensión de su comportamiento frente a cambios estructurales abruptos y al análisis de series temporales en escenarios de alta incertidumbre.
...
1. $E(y_t) = \mu_t = \mu$ \hfill (1)
2. $Var(y_t) = \sigma_t^2 = \sigma^2$ \hfill (2) 
3. $Cov(y_t, y_{t-k}) = \gamma_k \ \ \ \ \ k = 0, \pm 1, \pm 2, \text{etc}$ \hfill (3)
...
donde $\rho_k$ es el coeficiente de autocorrelación para el rezago $k$, $y_t$ y $y_{t+k}$ son los valores de la serie en los tiempos $t$ y $t+k$ respectivamente, $\gamma_k$ es la autocovarianza de rezago $k$ y notar que $Var(y_{t}) = Var(y_{t+k}) = \gamma_0$.

\setlength{\parindent}{1.5em}

El coeficiente $\rho_k$ varía de -1 a 1. Si $\rho_k$ está cerca de 1 indica que los valores de la serie están altamente correlacionados con los valores rezagados en $k$ momentos. Si $\rho_k$ está cerca de -1 indica que los valores de la serie están altamente correlacionados de forma negativa con los valores rezagados en $k$ momentos. Por último, si $\rho_k$ está cerca de 0 indica que no hay una correlación importante entre los valores de la serie para ese rezago.

#### Función de Autocorrelación Parcial

La Función de Autocorrelación Parcial (PACF, por sus siglas en inglés) permite medir la relación lineal directa entre una observación y su valor rezagado $k$, eliminando previamente el efecto de los rezagos intermedios $1, 2, ..., k-1$. A diferencia de la Función de Autocorrelación (ACF), que capta tanto efectos directos como indirectos entre los valores de la serie, la PACF aísla la contribución de cada rezago respecto de la de dichos rezagos intermedios, sin que ello suponga eliminar el efecto de los rezagos posteriores $k+1, k+2, ...$, que continúan contribuyendo a la relación entre las observaciones. La expresión de la autocorrelación parcial correspondiente al rezago $k$ resulta:

$$\phi_{kk} = Corr(y_t, y_{t+k}|y_{t+1}, y_{t+2},...,y_{t+k-1})$${#eq-pacf}
...
$$\hat{y}_{T+l|T} = E(y_{T+l}|y_T,y_{T-1},...)$${#eq-pron}
...
donde $y_t$ representa el valor observado, $\phi_p(B) = (1- \phi_1B - \cdots - \phi_pB^p)$ es el operador AR regular de orden p, $\nabla^d = (1-B)^d$ representa las diferencias regulares y $\theta_q(B) = (1 - \theta_1B - \cdots - \theta_qB^q)$ es el operador promedio móvil regular de orden q. Adicionalmente, se asume que el término de error $\varepsilon_t$ constituye un proceso ruido blanco gaussiano, es decir, $\varepsilon_t \underset{iid}{\sim} \mathcal{N}(0, \sigma^2_{\varepsilon})$.
...
donde $y_t$ representa el valor observado, $\Phi_P(B^s) = (1- \Phi_1B^s - \cdots - \Phi_PB^{sP})$ es el operador AR estacional de orden P, $\nabla_s^D = (1-B^s)^D$ representa las diferencias estacionales, $\Theta_Q(B^s) = (1 - \Theta_1B^s - \cdots - \Theta_QB^{sQ})$ es el operador promedio móvil estacional de orden Q y $\varepsilon_t$ es un proceso ruido blanco gaussiano.
...
![Ejemplo ilustrativo del proceso iterativo de búsqueda de los parámetros del algoritmo de Hyndman-Khandakar.](Imgs/algoritmo_hk.jpeg){#fig-algoritmohk}

La Figura 1 ilustra este procedimiento sobre el plano de órdenes regulares $(p, q)$, una vez fijado el orden de diferenciación $d$ a través del test de KPSS. El algoritmo no evalúa la totalidad de las combinaciones posibles —representadas por los puntos grises—, sino que recorre el espacio de manera dirigida. En el primer paso se estiman cuatro modelos iniciales: $ARIMA(0, d, 0)$, $ARIMA(1, d, 0)$, $ARIMA(0, d, 1)$ y $ARIMA(2, d, 2)$. El que arroja el menor $AICc$, señalado con un círculo, se adopta como modelo de referencia. En cada iteración subsiguiente se estiman las variaciones que modifican $p$ y/o $q$ en $\pm 1$ respecto del modelo vigente, y si alguna de ellas mejora el criterio, pasa a ocupar el lugar de referencia y define el centro de la búsqueda siguiente. Así, el recorrido avanza desde el modelo inicial hacia órdenes crecientes paso 2 y 3 hasta que, en el paso 4, ninguna de las variaciones exploradas logra reducir el $AICc$ y el procedimiento se detiene, quedando seleccionado el último modelo circundado.

Si bien los modelos ARIMA y SARIMA constituyen un punto de partida indispensable, presentan limitaciones importantes cuando la serie contiene eventos disruptivos. Bajo estas circunstancias, los parámetros estimados a partir de datos previos a la disrupción pueden no reflejar adecuadamente el comportamiento posterior. Asimismo, la adaptación del modelo al patrón estacional posterior puede ser lenta o inestable.
...
donde $\ell_t$ es el nivel (o el valor suavizado) de la serie en el tiempo $t$. Estableciendo $h=1$, se obtienen los valores ajustados, mientras que al establecer $t = T$, se obtienen los pronósticos reales, más allá de los datos de entrenamiento.
...
$$\ell_t = \alpha y_t + (1-\alpha)\ell_{t-1}$${#eq-suavizado}
...
$$\ell_t = \ell_{t-1} + \alpha(y_t - \ell_{t-1}) = \ell_{t-1} + \alpha\varepsilon_t$${#eq-suavizado2}
...
Para que esto se convierta en un modelo totalmente especificado es necesario indicar los supuestos distribucionales de los errores, los cuales son: $\varepsilon_t \underset{iid}{\sim} \mathcal{N}(0, \sigma^2_{\varepsilon})$ y por lo tanto, el modelo puede escribirse como:

$$y_t = \ell_{t-1} + \varepsilon_t$${#eq-suavizado3}

$$\ell_t = \ell_{t-1} + \alpha\varepsilon_t$${#eq-suavizado4}
...
Para este modelo se considera que $\varepsilon_t = y_t - (\ell_{t-1} + b_{t-1})$, $\varepsilon_t \underset{iid}{\sim} \mathcal{N}(0, \sigma^2_{\varepsilon})$ y siguiendo los pasos de la especificación anterior se obtiene:

\setlength{\parskip}{0.1em}

$$y_t = \ell_{t-1} + b_{t-1} + \varepsilon_t$${#eq-holt1}

$$\ell_t = \ell_{t-1} + b_{t-1} + \alpha\varepsilon_t$${#eq-holt2}

$$b_t = b_{t-1} + \alpha\beta^*\varepsilon_t$${#eq-holt3}
...
Para este modelo se considera que $\varepsilon_t = y_t - (\ell_{t-1} + b_{t-1} + s_{t-m})$, $\varepsilon_t \underset{iid}{\sim} \mathcal{N}(0, \sigma^2_{\varepsilon})$ y siguiendo los pasos de la especificación anterior se obtiene:

\setlength{\parskip}{0.1em}

$$y_t = \ell_{t-1} + b_{t-1} + s_{t-m} + \varepsilon_t$${#eq-holtw1}

$$\ell_t = \ell_{t-1} + b_{t-1} + \alpha\varepsilon_t$${#eq-holtw2}

$$b_t = b_{t-1} + \alpha\beta^*\varepsilon_t$${#eq-holtw3}

$$s_t = s_{t-m} + \gamma\varepsilon_t$${#eq-holtw4}
...
- **Enforque basado en modelos.** Utilizar los datos del periodo pre-disruptivo para estimar un modelo (ARIMA, ETS, etc.) y obtener a partir de él las observaciones estimadas durante el período disruptivo.

- **Enforque basado en medidas resumen.** Utilizar los datos del periodo pre-disruptivo para estimar las observaciones del periodo disruptivo mediante alguna medida resumen (media, mediana, entre otras).
...
$$\tilde{y}_{T+h|T} \pm c \cdot \tilde{\sigma}_h$${#eq-intpred}

\setlength{\parindent}{0pt}

donde $c$ es el cuantil de la distribución utilizada que depende de la probabilidad de cobertura deseada para calcular el intervalo de predicción y $\tilde{\sigma}_h$ representa una estimación del desvío estándar de la distribución predictiva combinada $h$ pasos hacia adelante. No obstante, en esta tesina los intervalos de predicción no se calculan bajo un supuesto paramétrico de normalidad, sino a partir de los cuantiles empíricos de la distribución predictiva obtenida mediante simulaciones de los distintos modelos que integran el *ensemble*.
...
$$
W_{\alpha} = \frac{1}{h} \cdot \sum_{t=1}^{h}W_{\alpha, t}
$${#eq-winkler2}
...
```{r graf 1}
#| label: fig-serie1
#| fig-cap: "Pasajeros transportados por el sistema de Transporte Urbano de Pasajeros en la ciudad de Rosario. Periodo 01/2006-12/2024."
#| fig-height: 2.7
#| fig-width: 7
...
```{r}
datos_pre_disr <- datos_series |> filter(mes_anio <= yearmonth("2020 feb"))
datos_post_disr <- datos_series |> filter(mes_anio >= yearmonth("2020 mar"))

lambda_guerrero_pre_disr <- forecast::BoxCox.lambda(datos_pre_disr$pasajeros, method = "guerrero", lower = -2, upper = 2)
lambda_guerrero_post_disr <- forecast::BoxCox.lambda(datos_post_disr$pasajeros, method = "guerrero", lower = -2, upper = 2)
lambda_guerrero <- forecast::BoxCox.lambda(datos_series$pasajeros, method = "guerrero")
```
...
En términos de desempeño predictivo, la @fig-pronostico1 presenta los pronósticos generados en cada una de las diferentes etapas. Durante el periodo del evento disruptivo, se observa que el modelo estimado no logra predecir adecuadamente la magnitud de la caída de la serie. Esto se debe a que estos modelos se basan en la información histórica, por lo que tienden a extrapolar esta información previa, replicando en cierta forma el comportamiento pasado. 
...
Los modelos ETS ajustados sobre la serie transformada en cada etapa corresponden a los especificados en la @tbl-ajuste2. Estas especificaciones permiten capturar la evolución del nivel, la tendencia y la estacionalidad de la serie, adaptándose a las distintas fases del periodo analizado.
...
A diferencia de los modelos ARIMA/SARIMA, los modelos ETS presentan una mayor capacidad de adaptación frente a cambios repentinos en la dinámica de la serie. Este comportamiento se evidencia en la @fig-pronostico2, donde se observa que, si bien durante el periodo del *shock* los pronósticos resultan similares a los obtenidos mediante modelos ARIMA/SARIMA, en la fase de recuperación los modelos ETS logran ajustarse con mayor rapidez a la nueva estructura de la serie, lo que se traduce en una mejora sustancial en la calidad de los pronósticos, reflejando una capacidad superior para capturar la recuperación gradual de la serie.
...
1 & \text{si} \ t \in \ \text{Agosto 2021 - Diciembre 2024} \\
0 & \text{en otro caso}\\
\end{matrix}\right.$${#eq-segvarindicadora}

```{r modelos_ajustados3}
#| tbl-cap: "Modelos ajustados en cada fase"
#| label: tbl-ajuste-mint

data.frame(
  entrenamiento = c("1", "2", "3", "4", "5"),
  modelos = c("$SARIMA(1,0,2)(1,1,0)_{12}$", "$SARIMA(0,1,1)(0,0,1)_{12}$", "$SARIMA(1,0,2)(0,0,2)_{12}$", "$SARIMA(1,0,2)(0,0,2)_{12}$", "$SARIMA(1,0,2)(0,0,2)_{12}$")
) |> 
  kable(col.names = c("Ventana", "Modelos ajustados"))
```
...
#| fig-cap: "Pasajeros transportados por el sistema de Transporte Urbano de Pasajeros en la ciudad de Rosario. El periodo entre Marzo 2020 a Julio 2021 se estableció como faltante"
...
```{r modelos_ajustados4}
#| tbl-cap: "Modelos ajustados en cada fase"
#| label: tbl-ajuste-stm

data.frame(
  entrenamiento = c("1", "2", "3", "4", "5"),
  modelos = c("$SARIMA(1,0,2)(1,1,0)_{12}$", "$SARIMA(1,0,2)(1,1,0)_{12}$", "$SARIMA(1,0,2)(1,1,0)_{12}$", "$SARIMA(1,0,2)(1,1,0)_{12}$", "$SARIMA(1,0,2)(0,1,0)_{12}$")
) |> 
  kable(col.names = c("Ventana", "Modelos ajustados"))
```
...
#| fig-cap: "Pasajeros transportados por el sistema de Transporte Urbano de Pasajeros en la ciudad de Rosario. El periodo entre Marzo 2020 a Julio 2021 se reconstruyó utilizando el promedio de cada mes con los 3 años previos a Marzo 2020."
...
datos_series_wmhb |> 
  mutate(Periodo = if_else((mes_anio >= yearmonth("2020 Mar") & mes_anio <= yearmonth("2021 Jul")), "Reconstrucción", "Original")) |> 
  ggplot() +
    aes(x = mes_anio, y = pasajeros) + 
    geom_line() +
    geom_point(aes(color = Periodo), size = 0.75) +
...
```{r modelos_ajustados5}
#| tbl-cap: "Modelos ajustados en cada fase"
#| label: tbl-ajuste-wmhb

data.frame(
  entrenamiento = c("1", "2", "3", "4", "5"),
  modelos = c("$SARIMA(1,0,2)(1,1,0)_{12}$", "$SARIMA(2,0,3)(0,1,1)_{12}$", "$SARIMA(2,0,0)(0,1,1)_{12}$", "$SARIMA(1,0,2)(0,1,1)_{12}$", "$SARIMA(1,0,1)(2,1,1)_{12}$")
) |> 
  kable(col.names = c("Ventana", "Modelos ajustados"))
```
...
## Resultados comparativos de pronóstico
...
```{r arima_sarima_ljung_box}
id = c(1, 2, 3, 4, 5)
lag = seq(12, 72, 12)
pos = 1
lb_arima_sarima <- data.frame()

for(i in id){
  for(j in lag){
    lb_arima_sarima <- rbind(lb_arima_sarima, ljung_box_arima(ajuste1, id = i, lag = j))
  }
}
```
...
for(i in id){
  for(j in lag){
    lb_ets <- rbind(lb_ets, ljung_box_ets(ajuste1, id = i, lag = j))
  }
}
```
...
for(i in id){
  for(j in lag){
    lb_mint <- rbind(lb_mint, ljung_box_arima(ajuste2, id = i, lag = j))
  }
}
```
...
for(i in id){
  for(j in lag){
    lb_stm <- rbind(lb_stm, ljung_box_arima(ajuste3, id = i, lag = j))
  }
}
```
...
for(i in id){
  for(j in lag){
    lb_wmhb <- rbind(lb_wmhb, ljung_box_arima(ajuste4, id = i, lag = j))
  }
}
```
...
```{r lb_arima_sarima1}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"

lb_arima_sarima |> 
  filter(.id == 1) |> 
  dplyr::select(lag, lb_stat, lb_pvalue) |> 
  kable(col.names = c("Lag", "Estadística", "P-Valor"), format = "pipe", digits = 4)
```

```{r normalidad_sarima1}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,0,2)(1,1,0)_{12}$"

res_m1 <- ajuste1$modelos |>
  filter(.id == 1) |> 
  dplyr::select(arima) |> 
  augment()

data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m1$.innov)[[1]], nortest::lillie.test(res_m1$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m1$.innov)[[2]], nortest::lillie.test(res_m1$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_arima_sarima2}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo $SARIMA(1,1,3)(0,0,2)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_sarima2}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,1,3)(0,0,2)_{12}$"

res_m2 <- ajuste1$modelos |>
  filter(.id == 2) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m2$.innov)[[1]], nortest::lillie.test(res_m2$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m2$.innov)[[2]], nortest::lillie.test(res_m2$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r normalidad_sarima3}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,1,3)(0,0,2)_{12}$"

res_m3 <- ajuste1$modelos |>
  filter(.id == 3) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m3$.innov)[[1]], nortest::lillie.test(res_m3$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m3$.innov)[[2]], nortest::lillie.test(res_m3$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_arima_sarima4}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo $SARIMA(1,1,3)(0,0,1)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_sarima4}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,1,3)(0,0,1)_{12}$"

res_m4 <- ajuste1$modelos |>
  filter(.id == 4) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m4$.innov)[[1]], nortest::lillie.test(res_m4$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m4$.innov)[[2]], nortest::lillie.test(res_m4$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r normalidad_sarima5}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,1,3)(0,0,2)_{12}$"

res_m5 <- ajuste1$modelos |>
  filter(.id == 5) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m5$.innov)[[1]], nortest::lillie.test(res_m5$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m5$.innov)[[2]], nortest::lillie.test(res_m5$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_ets1}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo $ETS(M,A,A)$"
...
```{r normalidad_ets1}
#| tbl-cap: "Tests de normalidad del modelo $ETS(M,A,A)$"

res_m6 <- ajuste1$modelos |>
  filter(.id == 1) |> 
  dplyr::select(ets) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m6$.innov)[[1]], nortest::lillie.test(res_m6$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m6$.innov)[[2]], nortest::lillie.test(res_m6$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_ets2}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo $ETS(A,N,A)$"
...
```{r normalidad_ets2}
#| tbl-cap: "Tests de normalidad del modelo $ETS(A,N,A)$"

res_m7 <- ajuste1$modelos |>
  filter(.id == 2) |> 
  dplyr::select(ets) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m7$.innov)[[1]], nortest::lillie.test(res_m7$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m7$.innov)[[2]], nortest::lillie.test(res_m7$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m8$.innov)[[1]], nortest::lillie.test(res_m8$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m8$.innov)[[2]], nortest::lillie.test(res_m8$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m9$.innov)[[1]], nortest::lillie.test(res_m9$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m9$.innov)[[2]], nortest::lillie.test(res_m9$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m10$.innov)[[1]], nortest::lillie.test(res_m10$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m10$.innov)[[2]], nortest::lillie.test(res_m10$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```

```{r residuos modelos11}
#| fig-cap: "Residuos modelo intervención $SARIMA(1,0,2)(1,1,0)_{12}$"
...
```{r lb_mint1}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo intervención $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m11$.innov)[[1]], nortest::lillie.test(res_m11$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m11$.innov)[[2]], nortest::lillie.test(res_m11$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```

```{r residuos modelos12}
#| fig-cap: "Residuos modelo intervención $SARIMA(0,1,1)(0,0,1)_{12}$"
...
```{r lb_mint2}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo intervención $SARIMA(0,1,1)(0,0,1)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_mint2}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(0,1,1)(0,0,1)_{12}$"

res_m12 <- ajuste2$modelos |>
  filter(.id == 2) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m12$.innov)[[1]], nortest::lillie.test(res_m12$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m12$.innov)[[2]], nortest::lillie.test(res_m12$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```

```{r residuos modelos13}
#| fig-cap: "Residuos modelo intervención $SARIMA(1,0,2)(0,0,2)_{12}$"
...
```{r lb_mint3}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo intervención $SARIMA(1,0,2)(0,0,2)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_mint3}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,0,2)(0,0,2)_{12}$"

res_m13 <- ajuste2$modelos |>
  filter(.id == 3) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m13$.innov)[[1]], nortest::lillie.test(res_m13$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m13$.innov)[[2]], nortest::lillie.test(res_m13$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m14$.innov)[[1]], nortest::lillie.test(res_m14$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m14$.innov)[[2]], nortest::lillie.test(res_m14$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m15$.innov)[[1]], nortest::lillie.test(res_m15$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m15$.innov)[[2]], nortest::lillie.test(res_m15$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_stm1}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo stm $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m16$.innov)[[1]], nortest::lillie.test(res_m16$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m16$.innov)[[2]], nortest::lillie.test(res_m16$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m17$.innov)[[1]], nortest::lillie.test(res_m17$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m17$.innov)[[2]], nortest::lillie.test(res_m17$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m18$.innov)[[1]], nortest::lillie.test(res_m18$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m18$.innov)[[2]], nortest::lillie.test(res_m18$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m19$.innov)[[1]], nortest::lillie.test(res_m19$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m19$.innov)[[2]], nortest::lillie.test(res_m19$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_stm5}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo stm $SARIMA(1,0,2)(0,1,0)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_stm5}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,0,2)(0,1,0)_{12}$"

res_m20 <- ajuste3$modelos |>
  filter(.id == 5) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m20$.innov)[[1]], nortest::lillie.test(res_m20$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m20$.innov)[[2]], nortest::lillie.test(res_m20$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_wmhb1}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo wmhb $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m21$.innov)[[1]], nortest::lillie.test(res_m21$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m21$.innov)[[2]], nortest::lillie.test(res_m21$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_wmhb2}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo wmhb $SARIMA(2,0,3)(0,1,1)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_wmhb2}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(2,0,3)(0,1,1)_{12}$"

res_m22 <- ajuste4$modelos |>
  filter(.id == 2) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m22$.innov)[[1]], nortest::lillie.test(res_m22$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m22$.innov)[[2]], nortest::lillie.test(res_m22$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_wmhb3}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo wmhb $SARIMA(2,0,0)(0,1,1)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_wmhb3}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(2,0,0)(0,1,1)_{12}$"

res_m23 <- ajuste4$modelos |>
  filter(.id == 3) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m23$.innov)[[1]], nortest::lillie.test(res_m23$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m23$.innov)[[2]], nortest::lillie.test(res_m23$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_wmhb4}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo wmhb $SARIMA(1,0,2)(0,1,1)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_wmhb4}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,0,2)(0,1,1)_{12}$"

res_m24 <- ajuste4$modelos |>
  filter(.id == 4) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m24$.innov)[[1]], nortest::lillie.test(res_m24$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m24$.innov)[[2]], nortest::lillie.test(res_m24$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```
...
```{r lb_wmhb5}
#| tbl-cap: "Test de Ljung-Box cada 12 rezagos del modelo wmhb $SARIMA(1,0,1)(2,1,1)_{12}$"
#| cap-location: "bottom"
...
```{r normalidad_wmhb5}
#| tbl-cap: "Tests de normalidad del modelo $SARIMA(1,0,1)(2,1,1)_{12}$"

res_m25 <- ajuste4$modelos |>
  filter(.id == 5) |> 
  dplyr::select(arima) |> 
  augment()
...
data.frame(
  test = c("Shapiro-Wilk", "Lilliefors"),
  est = c(shapiro.test(res_m25$.innov)[[1]], nortest::lillie.test(res_m25$.innov)[[1]]),
  pvalue = c(shapiro.test(res_m25$.innov)[[2]], nortest::lillie.test(res_m25$.innov)[[2]])
) |> kable(col.names = c("Test", "Estadística", "P-Valor"), row.names = FALSE, digits = 4, format = "pipe")
```

```{r resultados modelos 1}
#| fig-cap: "Evaluación de los pronósticos usando (RMSE) para cada enfoque"
...
resultados_unidos |> 
  ...
  scale_colour_manual(values = unname(palette.colors(palette = "Okabe-Ito")[-1])) +
...
#| fig-cap: "Evaluación de los pronósticos usando (MAE) para cada enfoque"
...
#| fig-cap: "Evaluación de los pronósticos usando (MAPE) para cada enfoque. Se omite el año 2020, cuyos valores cercanos al 780% comprimen la escala e impiden distinguir el desempeño relativo de las estrategias."

resultados_unidos |> 
  filter(pronostico != 2020) |> 
  ggplot() +
...
#| fig-cap: "Evaluación de los pronósticos usando (Puntuación de Winkler) para cada enfoque"
