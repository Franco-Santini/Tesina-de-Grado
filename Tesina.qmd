...
Para que esto se convierta en un modelo totalmente especificado es necesario indicar los supuestos distribucionales de los errores, los cuales son: $\varepsilon_t \underset{iid}{\sim} \mathcal{N}(0, \sigma^2_{\varepsilon})$ el modelo puede escribirse como:
...
Con el objetivo de evaluar la adecuación de los modelos ajustados, se realizó un análisis diagnóstico de los residuos asociados a cada una de las metodologías consideradas. En particular, se analizaron gráficamente los residuos estandarizados, las funciones de autocorrelación residual (ACF) y los histogramas correspondientes, complementando dicho análisis mediante el test de Ljung-Box para evaluar la presencia de autocorrelación residual significativa.

Los gráficos diagnósticos completos se presentan en el Anexo, junto con los resultados asociados a cada modelo analizado.

En el caso de los modelos ARIMA/SARIMA, los residuos presentaron un comportamiento compatible con los supuestos clásicos de ruido blanco, sin evidencia significativa de autocorrelación residual. Esto sugiere una adecuada especificación de la estructura temporal capturada por dichos modelos.

Por otro lado, algunos de los enfoques más adaptativos, particularmente ciertos modelos ETS y estrategias aplicadas durante el período disruptivo, presentaron leves desviaciones respecto de los supuestos clásicos de normalidad, homocedasticidad o independencia residual. Sin embargo, estas desviaciones resultan esperables en contextos caracterizados por alta volatilidad y cambios estructurales abruptos, como los observados durante la pandemia de COVID-19.

En este sentido, los resultados obtenidos muestran que un ajuste residual menos “ideal” desde el punto de vista teórico no necesariamente implica un peor desempeño predictivo. De hecho, algunos modelos que presentaron pequeñas desviaciones en sus diagnósticos lograron adaptarse con mayor rapidez a los nuevos patrones de comportamiento de la serie, obteniendo pronósticos más precisos durante la etapa de recuperación.
...
Los resultados obtenidos evidencian que los modelos tradicionales ARIMA/SARIMA, si bien constituyen una herramienta sólida y ampliamente utilizada, presentan limitaciones importantes frente a eventos disruptivos. En particular, durante el período en el que ocurre la disrupción y la etapa de recuperación, estos modelos mostraron una menor capacidad de adaptación a los nuevos patrones de comportamiento de la demanda, reflejado en mayores errores de predicción.

Por el contrario, los modelos altamente adaptativos (ETS) demostraron una mejor capacidad para ajustarse a las variaciones recientes de la serie, capturando con mayor rapidez las modificaciones en la tendencia y en la estacionalidad. Esta característica permitió obtener pronósticos más precisos durante las fases de recuperación y estabilización, especialmente en contextos donde los patrones históricos dejaron de ser representativos del comportamiento futuro.
...
Desde una perspectiva aplicada, los resultados obtenidos proporcionan herramientas útiles para organismos vinculados a la planificación y gestión del transporte urbano, particularmente para el **Ente de la Movilidad de Rosario (EMR)**. La identificación de metodologías más robustas frente a shocks exógenos permite mejorar la capacidad de respuesta ante situaciones extraordinarias, facilitando el proceso de toma de decisiones estratégicas y operativas en contextos de alta incertidumbre.
...
```{r lb_arima_sarima1}
#| tbl-cap: "Tabla del test de Ljung-Box modelo $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"
...
```{r lb_arima_sarima2}
#| tbl-cap: "Tabla del test de Ljung-Box modelo $SARIMA(1,1,3)(0,0,2)_{12}$"
#| cap-location: "bottom"
...
```{r lb_arima_sarima4}
#| tbl-cap: "Tabla del test de Ljung-Box modelo $SARIMA(1,1,3)(0,0,1)_{12}$"
#| cap-location: "bottom"
...
```{r lb_ets1}
#| tbl-cap: "Tabla del test de Ljung-Box modelo $ETS(M,A,A)$"
...
```{r lb_ets2}
#| tbl-cap: "Tabla del test de Ljung-Box modelo $ETS(A,N,A)$"
...
```{r lb_mint1}
#| tbl-cap: "Tabla del test de Ljung-Box modelo intervención $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"
...
```{r lb_mint2}
#| tbl-cap: "Tabla del test de Ljung-Box modelo intervención $SARIMA(0,1,1)(0,0,1)_{12}$"
#| cap-location: "bottom"
...
```{r lb_mint3}
#| tbl-cap: "Tabla del test de Ljung-Box modelo intervención $SARIMA(1,0,2)(0,0,2)_{12}$"
#| cap-location: "bottom"
...
```{r lb_stm1}
#| tbl-cap: "Tabla del test de Ljung-Box modelo stm $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"
...
```{r lb_stm5}
#| tbl-cap: "Tabla del test de Ljung-Box modelo stm $SARIMA(1,0,2)(0,1,0)_{12}$"
#| cap-location: "bottom"
...
```{r lb_wmhb1}
#| tbl-cap: "Tabla del test de Ljung-Box modelo wmhb $SARIMA(1,0,2)(1,1,0)_{12}$"
#| cap-location: "bottom"
...
```{r lb_wmhb2}
#| tbl-cap: "Tabla del test de Ljung-Box modelo wmhb $SARIMA(2,0,3)(0,1,1)_{12}$"
#| cap-location: "bottom"
...
```{r lb_wmhb3}
#| tbl-cap: "Tabla del test de Ljung-Box modelo wmhb $SARIMA(2,0,0)(0,1,1)_{12}$"
#| cap-location: "bottom"
...
```{r lb_wmhb4}
#| tbl-cap: "Tabla del test de Ljung-Box modelo wmhb $SARIMA(1,0,2)(0,1,1)_{12}$"
#| cap-location: "bottom"
...
```{r lb_wmhb5}
#| tbl-cap: "Tabla del test de Ljung-Box modelo wmhb $SARIMA(1,0,1)(2,1,1)_{12}$"
#| cap-location: "bottom"
